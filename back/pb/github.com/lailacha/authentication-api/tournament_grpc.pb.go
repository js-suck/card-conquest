...
func (c *tournamentServiceClient) SuscribeTournamentUpdate(ctx context.Context, in *TournamentRequest, opts ...grpc.CallOption) (TournamentService_SuscribeTournamentUpdateClient, error) {
	...
	stream, err := c.cc.NewStream(ctx, &TournamentService_ServiceDesc.Streams[0], TournamentService_SuscribeTournamentUpdate_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &tournamentServiceSuscribeTournamentUpdateClient{ClientStream: stream}
...
func _TournamentService_SuscribeTournamentUpdate_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(TournamentServiceServer).SuscribeTournamentUpdate(m, &tournamentServiceSuscribeTournamentUpdateServer{ServerStream: stream})
}
