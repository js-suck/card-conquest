...
class _CrudTournamentScreenState extends State<CrudTournamentScreen> {
  ...
  Future<void> _fetchTournaments() async {
    try {
      ...
      setState(() {
        tournaments = data.map<Tournament>((json) => Tournament.fromJson(json)).toList();
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _updateTournament(Tournament tournament) async {
    try {
      final response = await apiService.put('tournaments/${tournament.id}', tournament.toJson());
      if (response.statusCode == 200) {
        _fetchTournaments();
      } else {
  ...
  Future<void> _deleteTournament(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Failed to delete tournament. Status code: ${response.statusCode}');
      }
    } catch (e) {
      print('Error: $e');
    }
  ...
  void _showTournamentDialog(Tournament? tournament) {
    ...
    final _descriptionController = TextEditingController(text: tournament?.description ?? '');
    final _locationController = TextEditingController(text: tournament?.location ?? '');
    final _startDateController = TextEditingController(text: tournament?.startDate ?? '');
    final _endDateController = TextEditingController(text: tournament?.endDate ?? '');
    final _maxPlayersController = TextEditingController(text: tournament?.maxPlayers.toString() ?? '0');
    final _roundsController = TextEditingController(text: tournament?.rounds.toString() ?? '1');
              ...
              if (tournament == null) {
                _createTournament(
                  Tournament(
                    ...
                    organizer: Organizer(id: 1, name: 'Organizer Name', email: 'organizer@example.com'), // Placeholder
                    game: Game(id: 1, name: 'Game Name', imageUrl: ''), // Placeholder
                    tags: [],
